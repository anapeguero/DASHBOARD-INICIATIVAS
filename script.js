...
function buildModel(){
  ...
  const pendingSheet=projectConfig?.pendingSheet||'Puntos pendientes';
  const considerationSheet=projectConfig?.considerationsSheet||pendingSheet;
  const pen=pendingSheet?rows(pendingSheet):[];
  const cons=considerationSheet?rows(considerationSheet):[];
  const pre=rows('Pre-Liquidar');

  const totalRowIndex=Number.isInteger(projectConfig?.totalRowIndex)?projectConfig.totalRowIndex:6;
  const partStartIndex=Number.isInteger(projectConfig?.partStartIndex)?projectConfig.partStartIndex:20;
  const approvedUsdCol=Number.isInteger(projectConfig?.approvedUsdCol)?projectConfig.approvedUsdCol:4;
  const currentDopCol=Number.isInteger(projectConfig?.currentDopCol)?projectConfig.currentDopCol:8;
  const currentUsdCol=Number.isInteger(projectConfig?.currentUsdCol)?projectConfig.currentUsdCol:9;

  const totalBudget=num(val(plan[totalRowIndex],currentDopCol));
  const summary=(plan.slice(8,13)||[]).map(r=>({name:text(val(r,2)),value:num(val(r,currentDopCol))})).filter(x=>x.name&&x.value>=0);

  // Partidas maestras 1.00–9.00 únicamente. La posición de columnas
  // puede variar por proyecto y se define en projects.json.
  const planRate=num(val(bd[1],9))||60.7;
  const part19=(plan.slice(partStartIndex)||[])
    .map(r=>{
      const rawPart=num(val(r,0));
      const approvedUsd=num(val(r,approvedUsdCol));
      const currentUsd=num(val(r,currentUsdCol));
      const currentDop=num(val(r,currentDopCol));
      const approvedDop=approvedUsd*planRate;
      return {
        rawPart,part:rawPart,name:text(val(r,2)),
  ...
  const approvedBudgetUsd=num(val(plan[totalRowIndex],approvedUsdCol));
  const currentBudgetUsd=num(val(plan[totalRowIndex],currentUsdCol));
  const approvedBudgetDop=approvedBudgetUsd*planRate;
  const currentBudgetDop=num(val(plan[totalRowIndex],currentDopCol))||currentBudgetUsd*planRate;
  ...
  let considerations=[];
  if(pen.length){
    issues=pen.slice(1,13).map((r,idx)=>({row:idx+2,issue:text(val(r,0)),owner:text(val(r,1)),comment:text(val(r,2))})).filter(x=>x.issue);
    considerations=pen.slice(14,22).map(r=>text(val(r,0))).filter(Boolean);
  }
  if(!issues.length && cons.length && considerationSheet==='Consideraciones'){
    // Hispanoamérica tiene una hoja de Consideraciones, no una lista formal
    // de pendientes con responsable. Las mostramos como contexto, sin inventar owners.
    considerations=cons.slice(3).map(r=>text(val(r,1))||text(val(r,0))).filter(Boolean);
  }
...
function renderHome(){
  ...
  $('homeOpeningDate').textContent=od
    ?new Intl.DateTimeFormat('es-DO',{day:'2-digit',month:'short',year:'numeric'}).format(od)
    :(projectConfig?.openingLabel||'Fecha exacta pendiente');
}
function startCountdown(){
  ...
  if(!target){
    $('countdownDateLabel').textContent=projectConfig?.openingLabel||'Fecha exacta pendiente';
    $('cdDays').textContent='—';$('cdHours').textContent='—';$('cdMinutes').textContent='—';$('cdSeconds').textContent='—';
    $('countdownStatus').textContent='Día exacto de apertura pendiente';
    return;
  }
  $('countdownDateLabel').textContent=projectConfig?.openingLabel||new Intl.DateTimeFormat('es-DO',{day:'numeric',month:'long',year:'numeric'}).format(target);
  const tick=()=>{
...
function renderOverview(){
  ...
  const approvedCard=$('kpiApprovedBudget')?.closest('.kpi')?.querySelector('span');
  const currentCard=$('kpiBudget')?.closest('.kpi')?.querySelector('span');
  if(approvedCard)approvedCard.textContent=projectConfig?.baselineLabel||'Presupuesto aprobado';
  if(currentCard)currentCard.textContent=projectConfig?.currentLabel||'Presupuesto vigente';
  ...
  $('overviewDiscountOrders').textContent=`${projectConfig?.baselineLabel||'Base'} ${compactMoney(model.approvedBudgetDop)} → ${projectConfig?.currentLabel||'vigente'} ${compactMoney(model.currentBudgetDop)}`;
...
function renderLiquidation(){
  ...
  const disclaimer=document.querySelector('#page-liquidation .disclaimer');
  const hasProjectParams=rows('Pre-Liquidar').length>0;
  if(disclaimer){
    disclaimer.innerHTML=hasProjectParams
      ?'<b>Nota:</b> este módulo usa los parámetros de la hoja “Pre-Liquidar” del proyecto como referencia operativa. No sustituye una liquidación aduanal ni una cotización formal.'
      :'<b>Nota:</b> este proyecto no incluye una hoja “Pre-Liquidar”. El estimador conserva valores de referencia genéricos para simulación; antes de tomar una decisión deben cargarse los parámetros logísticos y aduanales propios del proyecto.';
  }
  calcLiquidation();
}
