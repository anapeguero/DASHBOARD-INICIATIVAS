...
function buildModel(){
  ...
  // Partidas maestras 1-9. El aprobado viene de "Plan detallado Aprobado"
  // y el vigente de "Plan Detallado en ejecución" de Plan OI.
  const planRate=num(val(bd[1],9))||63;
  const part19=(plan.slice(20)||[])
    .map(r=>{
      const part=Math.round(num(val(r,0)));
      ...
      return {
        part,name:text(val(r,2)),approvedUsd,currentUsd,currentDop,approvedDop,
        savingsUsd:approvedUsd-currentUsd,
        savingsDop:approvedDop-currentDop
      };
    })
    .filter(x=>x.part>=1&&x.part<=9&&Math.abs(num(x.part)-x.part)<.001)
...
function renderCashflow(){
  ...
  const paidDop=summaries.filter(x=>x.date<now).reduce((s,x)=>s+x.amountDop,0);
  ...
  // Los gráficos se mantienen en DOP para comparar monedas sobre una misma base.
  chart('cashflowChart','bar',summaries.map(x=>x.label),[{label:'Necesidad de caja (DOP)',data:summaries.map(x=>x.amountDop)}],{money:true,legend:false});
  let cum=0;const cumulative=summaries.map(x=>cum+=x.amountDop);
  chart('cashflowCumulativeChart','line',summaries.map(x=>x.label),[{label:'Necesidad acumulada (DOP)',data:cumulative,borderWidth:3,tension:.25,pointRadius:2}],{money:true,legend:false});
...
function renderOverview(){
  ...
  const partLabels=model.part19.map(x=>`${x.part}. ${x.name.replace(/^\d+(\.0+)?\s*/,'').trim()}`);
