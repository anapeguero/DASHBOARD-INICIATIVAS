...
function buildModel(){
  ...
  // Resumen ejecutivo del gráfico principal: partidas maestras 1, 2, 3 y 4 del Plan OI.
  const part14=(plan.slice(20)||[])
    .map(r=>({
      part:num(val(r,0)),
      name:text(val(r,2)),
      value:num(val(r,8))||num(val(r,6))
    }))
    .filter(x=>[1,2,3,4].includes(x.part))
    ...
    paymentDefs.forEach(def=>{
      ...
      if(period>0 && amount>0){
        ...
        plannedPayments.push({
          ...
          category,item:item||category,supplier,payment:def.name,amount
        });
      }
    });
  });

  return {plan,bd,cron,seg,pen,pre,totalBudget,summary,part14,purchases,scheduled,opening,logistics,issues,considerations,preParams,cashflowMonths,plannedPayments};
}
...
function paymentMonthSummary(){
  ...
  model.plannedPayments.forEach(p=>{
    ...
    if(!m.has(key))m.set(key,{date:p.date,label:p.monthLabel,amount:0,count:0});
    const x=m.get(key);x.amount+=p.amount;x.count++;
  });
...
function paymentAlertText(d){
  ...
  if(months<0)return 'Mes programado ya transcurrido';
  if(months===0)return 'Pago programado este mes';
  if(months===1)return 'Prepárate: pago el próximo mes';
...
function renderPaymentReminders(){
  ...
  const markup=list.map(x=>`<article class="payment-reminder ${paymentState(x.date)}">
    ...
    <h3>${esc(x.label)}</h3><b>${compactMoney(x.amount)}</b>
    <p>${x.count} pago${x.count===1?'':'s'} identificado${x.count===1?'':'s'} en BD Plan Detallado.</p>
  </article>`).join('');
  $('paymentReminderList').innerHTML=markup||'<span class="muted">No hay pagos programados.</span>';
  $('overviewPaymentReminders').innerHTML=list.map(x=>`<div class="reminder-mini ${paymentState(x.date)}"><span>${paymentAlertText(x.date)}</span><b>${esc(x.label)}</b><small>${compactMoney(x.amount)} · ${x.count} pagos</small></div>`).join('');
}
function renderCashflow(){
  const arr=model.cashflowMonths.filter(x=>x.amount>0);
  const total=arr.reduce((s,x)=>s+x.amount,0);
  const peak=[...arr].sort((a,b)=>b.amount-a.amount)[0];
  const now=monthStart(new Date());
  const future=arr.filter(x=>x.date>=now).sort((a,b)=>a.date-b.date);
  const next=future[0];
  const horizon=new Date(now.getFullYear(),now.getMonth()+3,1);
  const next90=future.filter(x=>x.date<horizon).reduce((s,x)=>s+x.amount,0);
  $('cfTotal').textContent=compactMoney(total);
  $('cfPeakMonth').textContent=peak?peak.fullLabel:'—';$('cfPeakAmount').textContent=peak?compactMoney(peak.amount):'—';
  $('cfNextMonth').textContent=next?next.fullLabel:'—';$('cfNextAmount').textContent=next?compactMoney(next.amount):'Sin pagos futuros';
  $('cf90Days').textContent=compactMoney(next90);

  chart('cashflowChart','bar',model.cashflowMonths.map(x=>x.label),[{label:'Pago programado',data:model.cashflowMonths.map(x=>x.amount)}],{money:true,legend:false});
  let cum=0;const cumulative=model.cashflowMonths.map(x=>cum+=x.amount);
  chart('cashflowCumulativeChart','line',model.cashflowMonths.map(x=>x.label),[{label:'Acumulado',data:cumulative,borderWidth:3,tension:.25,pointRadius:2}],{money:true,legend:false});

  renderPaymentReminders();
  const pay=model.plannedPayments.filter(x=>x.date>=new Date(now.getFullYear(),now.getMonth()-1,1)).sort((a,b)=>a.date-b.date||b.amount-a.amount);
  $('paymentTableCount').textContent=`${pay.length} pagos desde el período actual`;
  $('paymentTable').innerHTML=pay.map(x=>`<tr>
    <td><b>${esc(x.monthLabel)}</b></td><td>${esc(x.category)}</td><td>${esc(x.item)}</td><td>${esc(x.supplier)||'—'}</td>
    <td>${x.payment}</td><td class="num money">${money(x.amount)}</td>
    <td><span class="status-pill ${paymentState(x.date)==='current'?'warn':paymentState(x.date)==='overdue'?'bad':'good'}">${paymentAlertText(x.date)}</span></td>
  </tr>`).join('');
}
...
function renderOverview(){
  ...
  const discountRows=model.logistics.filter(x=>x.discount>0);
  const totalDiscountDop=discountRows.reduce((s,x)=>s+x.discountDop,0);
  const grossDiscountBaseDop=discountRows.reduce((s,x)=>{
    const fx=/EUR/i.test(x.currency)?(num(val(model.seg[5],14))||76.5):/USD/i.test(x.currency)?(num(val(model.seg[5],13))||63):1;
    return s+x.grossSubtotal*fx;
  },0);
  $('overviewDiscount').textContent=compactMoney(totalDiscountDop);
  $('overviewDiscountOrders').textContent=`${discountRows.length} orden${discountRows.length===1?'':'es'} con descuento registrado`;
  $('overviewDiscountPct').textContent=pct(grossDiscountBaseDop?totalDiscountDop/grossDiscountBaseDop*100:0);
  ...
  const sm=model.part14.map(x=>({
    name:partLabels[x.part]||x.name,
    value:x.value
  }));
...
function renderImports(){
  ...
  const discounts=arr.filter(x=>x.discount>0);
  const totalDiscount=discounts.reduce((s,x)=>s+x.discountDop,0);
  $('impDiscount').textContent=compactMoney(totalDiscount);
  $('impDiscountCount').textContent=`${discounts.length} orden${discounts.length===1?'':'es'} con descuento`;
  ...
  const discountSup=groupSum(discounts,x=>x.supplier||'Sin proveedor',x=>x.discountDop).sort((a,b)=>b.value-a.value).slice(0,8);
  chart('discountSupplierChart','bar',discountSup.map(x=>x.label.slice(0,22)),[{label:'Descuento DOP',data:discountSup.map(x=>x.value)}],{money:true,legend:false,extra:{indexAxis:'y'}});
  ...
  $('importsTableCount').textContent=`${arr.length} órdenes / líneas · ${discounts.length} con descuento`;
  $('importsTable').innerHTML=arr.map(x=>{
    ...
    const discountCell=x.discount>0
      ? `<span class="discount-cell">${money(x.discount)} ${esc(x.currency)}</span>`
      : '<span class="muted">—</span>';
    return `<tr class="${x.discount>0?'has-discount':''}">
      <td><b>${esc(x.supplier)}</b></td>
      <td>${esc(x.po)}</td>
      <td>${esc(x.description)}</td>
      <td>${esc(x.origin)}</td>
      <td>${esc(x.term)}</td>
      <td>${fmtDate(x.orderDate)}</td>
      <td>${fmtDate(x.eta)}</td>
      <td><span class="status-pill ${cls}">${stat}</span></td>
      <td class="num">${money(x.grossSubtotal)} ${esc(x.currency)}</td>
      <td class="num">${discountCell}</td>
      <td class="num money">${money(x.subtotal)} ${esc(x.currency)}</td>
      <td class="num money">${money(x.dop)}</td>
    </tr>`;
  }).join('');
}
...
function renderAll(){
  ...
  renderHome();renderOverview();setupBudgetFilters();renderBudget();renderCashflow();renderTracking();renderImports();renderPending();renderLiquidation();
}
